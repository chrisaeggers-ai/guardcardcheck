...
class CaliforniaAdapter extends BaseStateAdapter {
  ...
  _firstFromDcaName(name) {
    ...
    if (!raw) return '';
    // DCA usually returns "LAST, FIRST MIDDLE" in summary search rows.
  ...
  _firstNameMatches(nameFromApi, requestedFirst) {
    ...
    const apiFirstRaw = this._firstFromDcaName(nameFromApi);
    const apiFirst = this._normName(apiFirstRaw);
    if (!apiFirst) return false;

    // Strict first-name matching to avoid false positives like
    // "MACK" matching "JACK".
  ...
  async search(firstName, lastName) {
    ...
    const finalRows = rows.filter((r) => this._firstNameMatches(r.name, first));
