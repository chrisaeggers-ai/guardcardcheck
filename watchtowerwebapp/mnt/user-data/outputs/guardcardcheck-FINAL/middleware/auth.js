 ...
 * Used for free-tier routes where auth is optional
 */
...
/**
 * requirePlan(minPlan) — gate a route to a minimum plan tier
 * Usage: router.get('/batch', authMiddleware, requirePlan('business'), handler)
 */
...
/**
 * checkUsageLimit — verify user hasn't exceeded monthly search limit
...
function checkUsageLimit(db) {
  return async (req, res, next) => {
    ...
    // Paid plans: monthly limit (null = unlimited)
...
/**
 * apiKeyAuth — authenticate via API key (Enterprise tier)
 * Looks for: Authorization: Bearer gcc_live_xxxxx  or  X-API-Key: gcc_live_xxxxx
 */
